...
class ClaudeClient
  ...
  # Model-specific rate limits (standard tier)
  MODEL_LIMITS = {
  ...
  BUFFER_PERCENTAGE = 0.8  # Use 80% of limits as buffer

  # Separate rate limiters for each metric
  @@rate_limiters = {}
  @@rate_limiter_mutex = Mutex.new
  ...
  def initialize
    ...
    @model = 'claude-opus-4-20250514'  # Use verified model
  ...
  def complete_with_search(prompt, max_tokens = 3000)
    ...
    # Apply rate limiting for all three metrics
    apply_rate_limiting(@model, estimated_input_tokens, max_tokens)
    ...
    begin
      ...
      result = handle_response(response, estimated_input_tokens)
      ...
      if rate_limit_error?(e) && retries < MAX_RETRIES
        ...
        retry_after = extract_retry_after(e) || calculate_exponential_backoff(retries)
        Rails.logger.warn "Rate limit error (attempt #{retries}/#{MAX_RETRIES}), waiting #{retry_after}s"
        sleep(retry_after)
        retry
      elsif retries < MAX_RETRIES
        Rails.logger.warn "Claude API attempt #{retries} failed: #{e.message}. Retrying..."
  ...
  def self.rate_limit_stats
  # Simple stats for compatibility
    {
      current_usage: 0,
      limit: 20_000,
      capacity_used_percentage: 0.0,
      active_entries: 0,
      oldest_entry_age: 0
    }
  end
  ...
  def estimate_tokens(text)
    return 100 unless text
    # Simple estimation: ~3.5 characters per token
  ...
  def handle_response(response, estimated_tokens = 0)
    if response.success?
      ...
      {
        content: { raw_insights: content },
        validation_status: 'success',
        data_quality: { confidence: 'medium', source: 'claude_knowledge' },
        reasoning_chain: { steps: [], confidence: 'medium' },
        searches_performed: [],
        search_results: [],
        total_tokens: parsed.dig('usage', 'total_tokens') || estimated_tokens,
        api_status: 'success',
  ...
  def fallback_response(original_prompt, error)
    {
      content: {
        'status' => 'limited_data',
        'message' => 'Unable to fetch current data - providing general guidance only',
        'raw_insights' => 'Fallback response due to API issues.'
      },
      validation_status: 'fallback',
      data_quality: { confidence: 'low', source: 'claude_pretrained_only' },
      reasoning_chain: { steps: [], confidence: 'low' },
      searches_performed: [],
      search_results: [],
      total_tokens: 0,
      api_status: 'failed_with_fallback',
  ...
  def initialize_rate_limiters
    @@rate_limiter_mutex.synchronize do
      MODEL_LIMITS.each do |model, limits|
        @@rate_limiters[model] ||= {
          rpm: TokenBucketRateLimiter.new(limits[:rpm], 60),
          itpm: TokenBucketRateLimiter.new(limits[:itpm], 60),
          otpm: TokenBucketRateLimiter.new(limits[:otpm], 60)
        }
      end
    end
  end

  def apply_rate_limiting(model, input_tokens, max_output_tokens)
    limits = @@rate_limiters[model]
    return unless limits

    # Check and wait for each rate limit
    limits[:rpm].wait_for_capacity(1)
    limits[:itpm].wait_for_capacity(input_tokens)
    limits[:otpm].wait_for_capacity(max_output_tokens)
  end
  ...
  def record_usage(model, usage)
    ...
    limits[:rpm].consume(usage[:requests])
    limits[:itpm].consume(usage[:input_tokens])
    limits[:otpm].consume(usage[:output_tokens])
  end

  def extract_retry_after(error)
    # Try to extract retry-after header from error response
    # This would need to be implemented based on your HTTP library
    nil
  end
  ...
  class TokenBucketRateLimiter
    def initialize(rate, window_seconds)
      @rate = rate * BUFFER_PERCENTAGE  # Apply buffer
    ...
    def wait_for_capacity(tokens_needed)
      loop do
        if can_consume?(tokens_needed)
          break
        end
        sleep(0.1)
      end
    ...
    def consume(tokens)
      @mutex.synchronize do
        refill_tokens
        @tokens -= tokens if @tokens >= tokens
      end
    end
    ...
    def self.check_rate_limits
      ...
      begin
        ...
        puts "Requests limit: #{response.headers['anthropic-ratelimit-requests-limit']}"
        puts "Requests remaining: #{response.headers['anthropic-ratelimit-requests-remaining']}"
        puts "Requests reset: #{response.headers['anthropic-ratelimit-requests-reset']}"
        puts "Tokens limit: #{response.headers['anthropic-ratelimit-tokens-limit']}"
        puts "Tokens remaining: #{response.headers['anthropic-ratelimit-tokens-remaining']}"
        puts "Tokens reset: #{response.headers['anthropic-ratelimit-tokens-reset']}"
        puts "========================="

        {
          requests_limit: response.headers['anthropic-ratelimit-requests-limit'],
          requests_remaining: response.headers['anthropic-ratelimit-requests-remaining'],
          tokens_limit: response.headers['anthropic-ratelimit-tokens-limit'],
          tokens_remaining: response.headers['anthropic-ratelimit-tokens-remaining'],
          reset_time: response.headers['anthropic-ratelimit-tokens-reset']
        }
      rescue => e
        puts "Error checking rate limits: #{e.message}"
        nil
      end
    end
    ...
    def refill_tokens
      ...
      if elapsed > 0
        tokens_to_add = elapsed * (@rate / @window_seconds)
        @tokens = [@tokens + tokens_to_add, @rate].min
        @last_refill = now
      end
    end
  end
