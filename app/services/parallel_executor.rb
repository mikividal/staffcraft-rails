...
#     Rails.logger.info "Rate limit status: #{GeminiClient.rate_limit_stats}"
...
#     max_concurrent_threads = [agents.size, 1].min
...
#     claude_client = GeminiClient.new
#     response = claude_client.complete_with_search(prompt, max_tokens: agent_config[:tokens])
...
#     rate_limit_capacity = GeminiClient::TOKEN_LIMIT_PER_MINUTE * GeminiClient::BUFFER_PERCENTAGE
...
#     [pause, 30].min # Máximo 5 segundos
#   end
...
#     {
#       rate_limit_stats: GeminiClient.rate_limit_stats,
...
class ParallelExecutor
  def self.execute_agents(agents, form_data)
    Rails.logger.info "Executing #{agents.size} agents"

    results = {}

    # Ejecutar todos los agentes en paralelo con threads
    threads = agents.map do |agent_config|
      Thread.new do
        execute_single_agent(agent_config, form_data)
      end
    end
    ...
    agents.each_with_index do |agent_config, index|
      begin
        result = threads[index].value
        results[agent_config[:name]] = result
        Rails.logger.info "Agent #{agent_config[:name]} completed"
      rescue => e
        Rails.logger.error "Agent #{agent_config[:name]} failed: #{e.message}"
        results[agent_config[:name]] = create_error_result(agent_config, e.message)
      end
    end
  ...
  def self.execute_single_agent(agent_config, form_data)
    ...
    response = gemini_client.complete_with_search(prompt, max_tokens: agent_config[:tokens])
  ...
  def self.create_error_result(agent_config, error_message)
    {
