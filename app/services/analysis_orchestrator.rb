...
#     Rails.logger.info "Initial rate limit status: #{GeminiClient.rate_limit_stats}"
...
#     Rails.logger.error "Final rate limit status: #{GeminiClient.rate_limit_stats}"
...
#         rate_limit_info: GeminiClient.rate_limit_stats
#       }
#     )
#     raise
#   end
...
#     rate_stats = GeminiClient.rate_limit_stats
...
#     post_stats = GeminiClient.rate_limit_stats
...
#       response = gemini_client.complete_with_search(prompt, max_tokens: CRITIC_AGENT[:tokens])
...
#       gemini_client = GeminiClient.new
#       response = gemini_client.complete_with_search(prompt, max_tokens: SYNTHESIZER[:tokens])
...
#     final_rate_stats = GeminiClient.rate_limit_stats
...
#       # Try to find searches in different possible locations
#       searches = nil

#       # Check data_sources first (most likely location based on logs)
#       if result[:data_sources].is_a?(Hash)
#         searches = result[:data_sources][:searches] || result[:data_sources]["searches"]
#       elsif result[:data_sources].is_a?(String)
#         # If data_sources is a JSON string, parse it
#         begin
#           parsed_data = JSON.parse(result[:data_sources])
#           searches = parsed_data["searches"]
#         rescue JSON::ParserError
...
#       # Handle searches count
#       if searches.is_a?(Integer) && searches > 0
#         agent_name = result[:agent_name] || "Unknown Agent"
#         sources[:web_searches] << "#{agent_name}: #{searches} searches"
#       elsif searches.respond_to?(:any?) && searches.any?
#         sources[:web_searches].concat(searches)
#       end

#       # Rest of the method remains the same...
#       if result[:data_quality]&.dig(:sources)
...
#       if result[:data_quality]&.dig(:source) == 'gemini_knowledge'
...
class AnalysisOrchestrator
  PARALLEL_AGENTS = [
    { name: 'Market & Salary Intelligence', class_name: 'AiAgents::MarketIntelligence', tokens: 1200 },
    { name: 'Technology & Tools Performance', class_name: 'AiAgents::TechnologyPerformance', tokens: 1200 },
    { name: 'Implementation Feasibility', class_name: 'AiAgents::ImplementationFeasibility', tokens: 1000 },
    { name: 'ROI & Business Impact', class_name: 'AiAgents::RoiBusinessImpact', tokens: 1000 },
    { name: 'Risk & Compliance Analysis', class_name: 'AiAgents::RiskCompliance', tokens: 800 }
  ].freeze

  CRITIC_AGENT = { name: 'Critical Analysis', class_name: 'AiAgents::CriticAgent', tokens: 2000 }.freeze
  SYNTHESIZER = { name: 'Synthesizer', class_name: 'AiAgents::StrategicSynthesizer', tokens: 2000 }.freeze
  ...
  def execute
    ...
    # 1. Ejecutar agentes en paralelo
    parallel_results = execute_parallel_agents
    ...
    # 3. Ejecutar synthesizer
    final_results = execute_synthesizer(parallel_results, critic_results)

    # 4. Guardar todo
    save_final_results(parallel_results, critic_results, final_results)
  ...
  def execute_parallel_agents
    Rails.logger.info "Executing #{PARALLEL_AGENTS.count} agents in parallel"

    results = ParallelExecutor.execute_agents(PARALLEL_AGENTS, @form_data)

    # Guardar resultados de cada agente
    results.each do |agent_name, result|
      save_agent_result(agent_name, result)
  ...
  def execute_critic(parallel_results)
    Rails.logger.info "Executing critic agent"
    ...
    save_agent_result(CRITIC_AGENT[:name], critic_result)
  ...
  def execute_synthesizer(parallel_results, critic_results)
    Rails.logger.info "Executing synthesizer"

    enhanced_results = parallel_results.merge('Critic Validation' => critic_results[:output])
    ...
    save_agent_result(SYNTHESIZER[:name], synthesizer_result)
    @total_tokens += synthesizer_result[:tokens_used]

    response[:content] # Esto es lo que ve el usuario final
  end
  ...
  def save_final_results(parallel_results, critic_results, synthesis)
    final_results = {
      synthesis: synthesis,
      ...
      metadata: {
        total_tokens: @total_tokens,
        total_cost: @total_tokens * 0.00003,
        agents_count: PARALLEL_AGENTS.count + 2 # +critic +synthesizer
      }
    }
    ...
    Rails.logger.info "Analysis completed using #{@total_tokens} tokens"
  end
end
